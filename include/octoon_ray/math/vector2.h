#ifndef OCTOON_MATH_VECTOR2_H_
#define OCTOON_MATH_VECTOR2_H_

#include <octoon_ray/math/mathfwd.h>
...
namespace octoon
{
	namespace math
	{
		...
		template<typename T>
		inline T sqrDistance(const detail::Vector2<T>& v1, const detail::Vector2<T>& v2) noexcept
		{
			return length2(v1 - v2);
		}

		template <typename T>
		inline T min2(const detail::Vector2<T>& v) noexcept
		{
			return std::min(v.x, v.y);
		}

		template <typename T>
		inline T max2(const detail::Vector2<T>& v) noexcept
		{
